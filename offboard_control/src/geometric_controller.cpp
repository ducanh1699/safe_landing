#include "offboard_control/geometric_controller.h"
...
velocityCtrl::velocityCtrl(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
  ...
  referenceSub_ =
      nh_.subscribe("reference/setpoint", 1, &velocityCtrl::targetCallback, this, ros::TransportHints().tcpNoDelay());
  // flatreferenceSub_ = nh_.subscribe("reference/flatsetpoint", 1, &velocityCtrl::flattargetCallback, this,
  //                                   ros::TransportHints().tcpNoDelay());
  yawreferenceSub_ =
      nh_.subscribe("reference/yaw", 1, &velocityCtrl::yawtargetCallback, this, ros::TransportHints().tcpNoDelay());
  multiDOFJointSub_ = nh_.subscribe("command/trajectory", 1, &velocityCtrl::multiDOFJointCallback, this,
                                    ros::TransportHints().tcpNoDelay());
  mavstateSub_ = nh_.subscribe("mavros/state", 1, &velocityCtrl::mavstateCallback, this, ros::TransportHints().tcpNoDelay());
  mavposeSub_ = nh_.subscribe("mavros/local_position/pose", 1, &velocityCtrl::mavposeCallback, this,
                              ros::TransportHints().tcpNoDelay());
  mavtwistSub_ = nh_.subscribe("mavros/local_position/velocity_local", 1, &velocityCtrl::mavtwistCallback, this,
                               ros::TransportHints().tcpNoDelay());
  ctrltriggerServ_ = nh_.advertiseService("tigger_rlcontroller", &velocityCtrl::ctrltriggerCallback, this);
  cmdloop_timer_ = nh_.createTimer(ros::Duration(0.01), &velocityCtrl::cmdloopCallback,this);  // Define timer for constant loop rate
  statusloop_timer_ = nh_.createTimer(ros::Duration(1), &velocityCtrl::statusloopCallback, this);  // Define timer for constant loop rate
  ...
  nh_private_.param<double>("Kp_x", kpx_, 8.0);
  nh_private_.param<double>("Kd_x", kdx_, 8.0);
  nh_private_.param<double>("Ki_x", kix_, 10.0);

  nh_private_.param<double>("Kp_y", kpy_, 8.0);
  nh_private_.param<double>("Kd_y", kdy_, 8.0);
  nh_private_.param<double>("Ki_y", kiy_, 10.0);

  nh_private_.param<double>("Kp_z", kpz_, 8.0);
  nh_private_.param<double>("Kd_z", kdz_, 8.0);
  nh_private_.param<double>("Ki_z", kiz_, 10.0);

  nh_private_.param<double>("kp_yaw_", kpyaw_, 1.5);
  nh_private_.param<double>("kd_yaw_", kdyaw_, 1.5);
  nh_private_.param<double>("ki_yaw_", kiyaw_, 3.3);
...
void velocityCtrl::targetCallback(const geometry_msgs::TwistStamped &msg) {
...
void velocityCtrl::yawtargetCallback(const std_msgs::Float32 &msg) {
  if (!velocity_yaw_) mavYaw_ = double(msg.data);
}

void velocityCtrl::multiDOFJointCallback(const trajectory_msgs::MultiDOFJointTrajectory &msg) {
...
void velocityCtrl::mavstateCallback(const mavros_msgs::State::ConstPtr &msg) { current_state_ = *msg; }
...
void velocityCtrl::pubReferencePose(const Eigen::Vector3d &target_position, const Eigen::Vector4d &target_attitude) {
...
geometry_msgs::PoseStamped velocityCtrl::vector3d2PoseStampedMsg(Eigen::Vector3d &position,
...
Eigen::Vector3d velocityCtrl::controlPosition(const Eigen::Vector3d &target_pos, const Eigen::Vector3d &target_vel,
...
void velocityCtrl::computeBodyRateCmd(Eigen::Vector4d &bodyrate_cmd, const Eigen::Vector3d &a_des) {
  // Reference attitude
...
Eigen::Vector3d velocityCtrl::poscontroller(const Eigen::Vector3d &pos_error, const Eigen::Vector3d &vel_error) {
  Eigen::Vector3d a_fb =
...
Eigen::Vector4d velocityCtrl::acc2quaternion(const Eigen::Vector3d &vector_acc, const double &yaw) {
  Eigen::Vector4d quat;
...
Eigen::Vector4d velocityCtrl::attcontroller(const Eigen::Vector4d &ref_att, const Eigen::Vector3d &ref_acc,
...
Eigen::Vector4d velocityCtrl::jerkcontroller(const Eigen::Vector3d &ref_jerk, const Eigen::Vector3d &ref_acc,
...
Eigen::Vector4d velocityCtrl::geometric_attcontroller(const Eigen::Vector4d &ref_att, const Eigen::Vector3d &ref_acc,
                                                       Eigen::Vector4d &curr_att) {
  // Geometric attitude controller
...
bool velocityCtrl::ctrltriggerCallback(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res) {
...
// void velocityCtrl::dynamicReconfigureCallback(offboard::GeometricControllerConfig &config,
//                                                uint32_t level) {
