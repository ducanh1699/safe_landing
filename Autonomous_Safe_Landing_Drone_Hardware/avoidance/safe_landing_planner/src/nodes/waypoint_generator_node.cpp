...
WaypointGeneratorNode::WaypointGeneratorNode(const ros::NodeHandle &nh) : nh_(nh), spin_dt_(0.1) {
  ...
  state_sub_ = nh_.subscribe("/mavros/state", 5, &WaypointGeneratorNode::stateCallback, this);
  grid_sub_ = nh_.subscribe("/grid_slp", 1, &WaypointGeneratorNode::gridCallback, this);
  trigger_safe_landing_ = nh_.subscribe("/trigger_safe_landing",10, &WaypointGeneratorNode::CheckAllowSafeLanding_Callback, this);
...
void WaypointGeneratorNode::CheckAllowSafeLanding_Callback(const std_msgs::Bool &msg){
  trigger_msg_ = msg.data;
  ROS_INFO("[ INFO] --------------- LAND ---------------\n");
  if (trigger_msg_){
...
void WaypointGeneratorNode::stateCallback(const mavros_msgs::State &msg) {
  if (msg.mode == "AUTO.LAND") {
    ...
    // ROS_INFO_STREAM("land trigger" << waypointGenerator_.is_land_waypoint_);
...
void WaypointGeneratorNode::publishTrajectorySetpoints(const Eigen::Vector3f &pos_sp, const Eigen::Vector3f &vel_sp,
  ...
  // ROS_INFO_STREAM("x_p " << pos_sp.x() << "y_p " << pos_sp.y() << "z_p " << pos_sp.z());
  // ROS_INFO_STREAM("x_v " << vel_sp.x() << "y_v " << vel_sp.y() << "z_v " << vel_sp.z());
